...
import com.touchme.common.MatchFilterVisitor;
import com.touchme.controller.BroadcastsHandler;
import com.touchme.controller.PhoneStateHandler;
import com.touchme.dirchooser.DirChooserConstants;
import com.touchme.model.Instance;
import com.touchme.model.Track;
import com.touchme.persistance.framework.db.ObservableDatabase;
import com.touchme.persistance.framework.filter.Filter;
import com.touchme.persistance.turtle.db.TurtleDatabase;
import com.touchme.persistance.turtle.db.structure.Tables;
import com.touchme.player.ObservableOutput;
import com.touchme.player.Output;
import com.touchme.player.OutputCommand;
import com.touchme.player.OutputUsingOnClickListener;
import com.touchme.playlist.Playlist;
import com.touchme.playlist.playorder.PlayOrderRandom;
import com.touchme.playlist.playorder.PlayOrderSorted;
import com.touchme.playlist.playorder.PlayOrderStrategy;
import com.touchme.preferences.AbstractKey;
import com.touchme.preferences.Keys;
import com.touchme.preferences.Preferences;
import com.touchme.preferences.PreferencesObserver;
import com.touchme.util.Shorty;
import com.touchme.view.AlbumArtView;
import com.touchme.view.FileChooser;
...
public class Player extends ListActivity {
	...
	private PlayOrderStrategy standartPlayOrderStrategy; // default interactive
															// next/prev
															// strategy
	private PlayOrderStrategy shufflePlayOrderStrategy;
	private PlayOrderStrategy playOrderStrategy; // strategy after tracks is
													// over, normally one of the
													// above
	...
	protected void onDestroy() {
		...
		tp.player.connectPlayer(new OutputCommand() {
			public void connected(Output output) {
				tp.playlist.preferences.set(Keys.EXIT_PLAY_TIME,
						output.getCurrentMillis());
				tp.playlist.preferences.set(Keys.FILTERS,
						tp.playlist.getFilter());
			}
		});
	...
	private void SetupApplication() {
		...
		playOrderStrategy = tp.playlist.preferences.get(Keys.SHUFFLE) ? shufflePlayOrderStrategy
				: standartPlayOrderStrategy;
	}
	...
	private void SetupButtons() {
		shuffleButton.setImageDrawable(tp.playlist.preferences
				.get(Keys.SHUFFLE) ? getResources().getDrawable(
				R.drawable.dice48_active) : getResources().getDrawable(
				R.drawable.dice48));

		shuffleCheckBox.setChecked(tp.playlist.preferences.get(Keys.SHUFFLE));

		mediaDir.setText(tp.playlist.preferences.getExitstingMediaPath()
				.toString());
	...
	private void SetupButtonListeners() {
		...
		backButton
				.setOnClickListener(new OutputUsingOnClickListener(tp.player) {
					@Override
					public void onClick(View v, Output output) {
						output.play(tp.playlist.getPrevious(
								standartPlayOrderStrategy,
								output.getCurrTrack()));
					}
				});
		...
		nextButton
				.setOnClickListener(new OutputUsingOnClickListener(tp.player) {
					@Override
					public void onClick(View v, Output output) {
						output.play(tp.playlist.getNext(
								standartPlayOrderStrategy,
								output.getCurrTrack()));
					}
				});

		shuffleButton.setOnClickListener(new OutputUsingOnClickListener(
				tp.player) {
			@Override
			public void onClick(View v, Output output) {
				output.play(tp.playlist.getNext(shufflePlayOrderStrategy,
						output.getCurrTrack()));
			}
		});
		shuffleButton.setOnLongClickListener(new View.OnLongClickListener() {
			public boolean onLongClick(View v) {
				tp.playlist.preferences.set(Keys.SHUFFLE,
			...
			public void onClick(View v) {
				...
				dirChooserIntent
						.putExtra(
								...
								tp.playlist.preferences.getExitstingMediaPath()
										.toString());
				Player.this.startActivityForResult(dirChooserIntent,
						DIR_CHOOSER_REQUEST);
			}
		});
	}
	...
	private void SetupObservers() {
		...
		tp.playlist.addObserver(new Playlist.PlaylistTrackChangeObserver() {
			public void startUpdatePlaylist() {
				runOnUiThread(new Runnable() {
					public void run() {
						...
						rescan.setImageDrawable(getResources().getDrawable(
								R.drawable.fs_scan_stop48));
			...
			public void startRescan(final int toProcess) {
				...
				runOnUiThread(new Runnable() {
					public void run() {
						...
						rescanTogglePause.setImageDrawable(getResources()
								.getDrawable(R.drawable.fs_scan_pause48));
						rescanProgressBarIndicatorState.setText("");
						rescanProgressBarIndicatorAll.setText(String
								.valueOf(rescanProgressBar.getMax()));
			...
			public void trackAdded(final String filePath,
					final int allreadyProcessed) {
				...
				if (!trackAddedhandler.hasMessages(0)) {
					trackAddedhandler.postDelayed(new Runnable() {
						public void run() {
							...
							rescanProgressBarIndicatorState.setText(String
									.valueOf(rescanProgressBar.getProgress()));
			...
			public void pauseRescan() {
				runOnUiThread(new Runnable() {
					public void run() {
						rescanTogglePause.setImageDrawable(getResources()
								.getDrawable(R.drawable.fs_scan_unpause48));
					}
				});
			}
			...
			public void filterAdded(final Filter<? super Tables.Tracks> filter) {
				tp.player.connectPlayer(new OutputCommand() {
					public void connected(Output output) {
						...
						if (currTrack == null
								|| !filter
										.accept(new MatchFilterVisitor<Track, Tables.Tracks>(
												currTrack))) {
							output.change(tp.playlist.getNext(
									playOrderStrategy, null));
						}
					}
				});
			}
		...
		tp.playlist.preferences.addObserver(new PreferencesObserver() {
			public void changed(AbstractKey<?, ?> key) {
				if (key.equals(Keys.SHUFFLE)) {
					final boolean shuffle = tp.playlist.preferences
							.get(Keys.SHUFFLE);
					playOrderStrategy = shuffle ? shufflePlayOrderStrategy
							: standartPlayOrderStrategy;
					// Update UI states
					runOnUiThread(new Runnable() {
						public void run() {
							shuffleButton.setImageDrawable(getResources()
									.getDrawable(
											shuffle ? R.drawable.dice48_active
													: R.drawable.dice48));
		...
		tp.player.addObserver(new ObservableOutput.PlayerObserver() {
			public void trackChanged(Track track, int lengthInMillis) {
				...
				tp.playlist.preferences.set(Keys.LAST_TRACK_PLAYED,
						track.getFullPath());
			}
			...
			public void run() {
				tp.player.connectPlayer(new OutputCommand() {
					public void connected(Output output) {
						...
						currTrackPosition.setText(ConvertToMinutes(output
								.getCurrentMillis()));
			...
			public void connected(final Output output) {
				output.setOnCompletionListener(new OnCompletionListener() {
					public void onCompletion(MediaPlayer mplayer) {
						output.play(tp.playlist.getNext(playOrderStrategy,
								output.getCurrTrack()));
					}
				});
			}
		...
		tp.db.addObserver(new ObservableDatabase.DbObserver() {
			...
			public void cleared() {
				Toast.makeText(getApplicationContext(),
						getString(R.string.toastRescan), Toast.LENGTH_LONG)
						.show();
	...
	private void resetLastTrack() {
		Set<Filter<? super Tables.Tracks>> filtersFromPref = tp.playlist.preferences
				.get(Keys.FILTERS);
		for (Filter<? super Tables.Tracks> filter : Shorty.avoidNull(
				filtersFromPref, new HashSet<Filter<? super Tables.Tracks>>())) {
			tp.playlist.addFilter(filter);
		}

		tp.player.connectPlayer(new OutputCommand() {
			public void connected(Output output) {
				String lastTrack = tp.playlist.preferences
						.get(Keys.LAST_TRACK_PLAYED);
				...
				if (restoredTrack == null) {
					...
					output.goToMillis(tp.playlist.preferences
							.get(Keys.EXIT_PLAY_TIME));
				}
			}
		});
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		final Track trackSelected = fileChooser.choose((Instance) l
				.getItemAtPosition(position));
		if (trackSelected != null) {
			if (!tp.playlist.getCompressedFilter()
					.accept(new MatchFilterVisitor<Track, Tables.Tracks>(
							trackSelected))) {
				tp.playlist.clearFilters();
			}
	 ...
	 * {@link com.touchme.playlist.Playlist.PlaylistObserver} to receive changes
	 */
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == DIR_CHOOSER_REQUEST) {
			if (resultCode == RESULT_OK) {
				...
				mediaDir.setText(tp.playlist.preferences
						.getExitstingMediaPath().toString());
				rescan();
			}
		} else {
